...
public class GraphicRaycast : BaseInputModule //BaseInputModule 클래스 상속
{
    ...
    {
        Static.graphicRaycastScript = this;
    }
    ...
    private void Update()
    {
        ...
        if (raycastResults.Count > 0) //충돌한 UI가 있으면
        {
            foreach (RaycastResult raycastResult in raycastResults) //충돌한 UI 탐색
            {
                ...
                if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || Input.GetMouseButtonDown(0)) //임의의 버튼을 클릭하면
