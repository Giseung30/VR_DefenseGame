...
{
    [Header("Variable")]
    public float rifleDamage; //Rifle 데미지
    public float fireDelay; //발사 지연시간

    [Header("Component")]
    public Transform firePosTransform; //발사 위치 Transform 정보
    public Animator rifleAnimator; //Rifle Animator 정보

    [Header("Line Renderer")]
    public LineRenderer rifleLineRenderer; //Rifle Renderer 정보
    ...
    private IEnumerator DrawLine()
    {
        ...
        while (true)
        {
            rifleLineRenderer.SetPosition(0, firePosTransform.position); //시작 지점 갱신
            rifleLineRenderer.SetPosition(1, firePosTransform.position + firePosTransform.forward * 100); //끝 지점 갱신
            ...
            if (weapon.activeSelf) //무기 오브젝트가 활성화되면
            {
                if (OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger) || Input.GetMouseButton(0)) //트리거를 당기면
                {
                    if (remainBullet > 0 && remainReloadTime <= 0f) //장탄 수가 남아있거나 장전중이 아니면
                    {
                        ...
                        rifleAnimator.Play("Shoot"); //발사 애니메이션 실행
                        ...
                        if (Physics.Raycast(firePosTransform.position, firePosTransform.forward, out hit, 100f)) //레이캐스트가 닿으면
                        {
                            ...
                            {
                                hit.transform.GetComponent<Enemy>().Damaged(rifleDamage); //데미지 함수 실행
